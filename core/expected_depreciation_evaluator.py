...
field_main          = "expected_depreciation"       # 本命字段：贬值金额，越低越好（负数）

top_k               = 5                             # 贬值前 top_k 排名才推荐
min_sample_size     = 5                             # 最小样本数，小于此值不做判断

# ======== 输出变量 ========
output = {
    "field":        field_main,                     # 当前评估字段名
    "value":        None,                           # 当前车辆的贬值金额
    "rank":         None,                           # 在同组内的排名
...
def evaluate(df: pd.DataFrame, row: pd.Series) -> dict:
    ...
    if sample_size < min_sample_size:
        ...
        is_value = rank <= top_k
        msg      = (
            f"贬值金额排第 {rank}，进入前 {top_k}，值得推荐"
            if is_value else
            f"贬值金额为 {value}，排第 {rank}，未进前 {top_k}"
        )
