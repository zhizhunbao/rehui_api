...
class ExpectedDepreciationEvaluator:
    # ======== 参数变量（字段名 & 判断逻辑） ========
    full_key        = "full_key"                    # 品牌+型号+配置（用于分组）
    year            = "year"                        # 车型年份（用于分组）
    url             = "url"                         # 页面链接字段
    main_field      = "expected_depreciation"       # 本命字段：贬值金额，越低越好（负数）

    top_k           = 5                             # 贬值最严重前 top_k，才算值
    min_sample_size = 5                             # 最小样本数，小于此值不参与判断
    ...
    def evaluate(cls, df: pd.DataFrame, row: pd.Series) -> dict:
        ...
        if total < cls.min_sample_size:
            ...
            msg = (
                f"Ranked #{rank} in expected depreciation — top {cls.top_k} value pick"
                if is_value else
                f"Depreciation amount is {value}, ranked #{rank}, not in top {cls.top_k} (lower is better)"
            )
