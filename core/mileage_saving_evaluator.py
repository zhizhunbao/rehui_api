...
field_main          = "mileage_saving"          # 本命字段：里程回血金额，越高越好

top_k               = 5                         # 排名前 top_k 才推荐
min_sample_size     = 5                         # 最小样本数，小于此值不判断

# ======== 输出变量 ========
output = {
    "field":        field_main,                 # 当前评估字段
    "value":        None,                       # 当前字段值
    "rank":         None,                       # 排名
    "sample_size":  None,                       # 样本量
...
def evaluate(df: pd.DataFrame, row: pd.Series) -> dict:
    ...
    if sample_size < min_sample_size:
        ...
        # ✅ 数值越高越推荐（从大到小排序）
        rank     = (group.sort_values(field_main, ascending=False)[field_main] >= value).sum()
        is_value = rank <= top_k
        msg      = (
            f"里程回血金额第 {rank} 名，进入前 {top_k}，值得推荐"
            if is_value else
            f"里程回血为 ${value}，排第 {rank} 名，未进前 {top_k}"
        )
